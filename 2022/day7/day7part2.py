...
class TreeInfo2:
	def __init__(self):
		self.subTreeSum = 0
		self.finalAnswer = 100000000000
		self.requiredDeletion = -1

def treeWalk2(tree, treeInfo):
	if(is_integer(tree.name)):
		treeInfo.subTreeSum = int(tree.name)
		return
	thisSubTree = 0
	for child in tree.children:
		treeWalk2(child, treeInfo)
		thisSubTree += treeInfo.subTreeSum
	treeInfo.subTreeSum = thisSubTree
	if(treeInfo.subTreeSum >= requiredDeletion and treeInfo.subTreeSum < treeInfo.finalAnswer):
...
freeSpace = 70000000 - treeInfo.subTreeSum
requiredDeletion = 30000000 - freeSpace

treeInfo = TreeInfo2()
treeInfo.requiredDeletion = requiredDeletion

# Start walking the tree
treeWalk2(treeRoot, treeInfo)
